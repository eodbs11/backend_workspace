/*
	인덱스(INDEX)
    - SQL 명령문의 처리속도를 향상시키기 위해 행들의 위치정보를 가지고 있다.
    
    * 데이터 검색 방식
    - TABLE Full Scan : 테이블 데이터를 처음부터 끝까지 검색하여 원하는 데이터를 찾는 방식
    - Indax Scan : 인덱스를 통해 데이터를 찾는 방식
...
/*
	함수(FUNCTION)
    - 프로시저와 거의 유사한 용도로 값을 반환하는 재사용 가능한 코드블럭
...
/*
	트리거(TRIGGER)
    -특정 이벤트가 발생할 떄 자동으로 실행될 내용을 정의하여 저장
    
    ex)
    - 회원탈퇴시 기존 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 
	  테이블에 자동으로 INSERT 처리
	- 신고횟수가 일정 수를 넘었을 때 해당회원을 블랙리스트로 처리
...
-- employee 테이블에서 부서 코드가 D6인 직원들의 부서코그를 D3으로 변경
UPDATE code
SET dept_code = 'D3'
WHERE dept_code = 'D6';
